      ...
      {design.apis.map((api) => (
        <article key={`${api.method}-${api.path}`} className="rounded-xl border border-border p-5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <code className="text-sm text-zinc-100">{api.path}</code>
          </div>
          <p className="mt-2 text-sm text-zinc-400">{api.description}</p>
...
function Block({ title, value }: { title: string; value: unknown }) {
  return (
    <div>
      ...
      <pre className="mt-1 overflow-x-auto rounded-md border border-border bg-zinc-950 p-3 text-xs text-zinc-300">
